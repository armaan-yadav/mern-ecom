...
export const allCoupons = tryCatch(async (req, res, next) => {
    ...
    if (!coupons)
        throw new ErrorHandler("something went wrong while fethcing coupons. try again", 500);
...
export const deleteCoupon = tryCatch(async (req, res, next) => {
    ...
    if (!response)
        throw new ErrorHandler("No coupon exists with such id", 404);
...
export const applyCoupon = tryCatch(async (req, res, next) => {
    ...
    if (!coupon)
        throw new ErrorHandler("Coupon code not provided", 400);
