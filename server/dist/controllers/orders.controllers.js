...
export const getOrderById = tryCatch(async (req, res, next) => {
    ...
    if (!order)
        throw new ErrorHandler("No  order  exists with given it", 404);
...
export const editOrder = tryCatch(async (req, res, next) => {
    ...
    if (!order)
        throw new ErrorHandler("No order exists with given id", 404);
    await invalidateCache({
        products: true,
        orders: true,
        admin: true,
...
export const deleteOrder = tryCatch(async (req, res, next) => {
    ...
    if (!order)
        throw new ErrorHandler(`no order exists with orderId ${id}`, 404);
    await invalidateCache({
        products: true,
        orders: true,
        admin: true,
